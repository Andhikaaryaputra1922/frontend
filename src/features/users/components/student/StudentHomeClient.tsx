      ...
      .then(data => {
        ...
        if (list.length === 0) {
          ...
          setPackages(list.map((p: any) => ({
            ...
            originalPrice: Number(p.price) * 1.5, // Mocking discount for UI similarity
            image: p.packageCourses?.[0]?.course?.thumbnailUrl || "https://images.unsplash.com/photo-1627556704302-624286467c65?auto=format&fit=crop&q=80&w=800",
            materialsCount: p.packageCourses?.length || 0
          })));
        }
      })
      .catch(err => {
        console.error("Dashboard Fetch Error:", err);
        // Fallback to empty state on error to avoid blank screen
        setPackages([]);
      })
